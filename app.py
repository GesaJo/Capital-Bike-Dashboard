...
from helper_functions import df_customer, get_weekday
...
def update_basic_graph(year, month, weekday, customer, check, year1, month1, day1, df=df):
    """function to render and update main graph"""

    df1, color1 = df_customer(df, customer)
    color2 = color1
    ...
    if check == "yes":
        ...
        if customer == "all":
            v, w, x, y, color1, color2 = day_stacked(day1, month1, year1, df1)
        else:
            v, w, x, y = day_single(day1, month1, year1, df1)
    else:
        if year == 2018:
            df1 = df_2018
        elif year == 2019:
        ...
        if customer == "all":
            df_w = get_weekday(weekday, df1)
            v, w, x, y, color1, color2 = months_stacked(month, df_w)
        else:
            df_w = get_weekday(weekday, df1)
...
def update_map(month, year, single_station, customer, df=df):
    """function to render and update map-graph"""
...
def most_used_stations(year, month, day, customer, df=df, df_loc=df_loc):
    """function to display most used station per day/month/year and number
        of bikes rented from that station"""
...
def draw_sankey(value):
    """function to render the sankey for most important routes"""
...
def weekday_graph(dummy):
    """function to render the static graph for number of rentals per
        weekday and hour"""
...
def weather_graph(weather, month, df=df):
    """function to render a graph according to weather-conditions"""
...
def weekday_graph2(weather, df=df):
    """function to render graph that shows average duration of rides in
...
def customer_graph(weather, df=df):
    """function to render graph that shows the number of rides per customer
        status over time"""
